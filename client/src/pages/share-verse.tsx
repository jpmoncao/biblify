...
export default function ShareVerse() {
    ...
    const handleShareImage = async () => {
        ...
        const file = new File([blob], "biblify-versiculo.png", { type: "image/png" });
    ...
    return (
        <div className={`${colorBackground} transition-all h-screen overflow-y-auto w-full fixed top-0 left-0 z-[50]`}>
            <Header title="Compartilhar" className={`${colorBackground} transition-all border-none ${isCapturing ? "hidden" : ""}`} />
            <main className={`mt-[5rem] w-full px-8 flex flex-col max-w-[480px] h-[80vh] overflow-hidden mx-auto ${isCapturing ? "shadow-lg shadow-black/70 py-4 rounded-lg sm:border border-black/70" : ""} `}>
                <div className="w-full flex justify-between mb-2">
                    <h1 className="text-2xl font-bold">
                        {bookName} {bookChapter} <span className="text-base">({bookVersion.toUpperCase()})</span>
                    </h1>

                    <Button
                        onClick={handleCopy}
                        className={`group ${isCapturing ? "hidden" : ""} ${colorBackground} transition-all border border-b-2 border-primary text-primary hover:text-white hover:bg-primary`}
                    >
                ...
                <div className={`flex flex-wrap justify-center items-center w-full my-6 gap-1 ${isCapturing ? "hidden" : ""}`}>
                    <Button
                    ...
                    >
                        <Share className={`text-primary group-hover:text-white`} />
                        Compartilhar
                    </Button>
                ...
                <div className={`${isCapturing ? "" : "hidden"} w-full flex justify-center mt-auto`}>
